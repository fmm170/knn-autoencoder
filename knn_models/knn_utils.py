...
class KnnSearch:
    def __init__(self, cfg: KnnConfig):
        self.cfg = cfg
        # self.num_pos_saved_tokens = 0
        # self.num_neg_saved_tokens = 0
    ...
    def setup_knn_search(self):
        ...
        self.cfg.load_keys = True
        if self.cfg.load_keys:
            datastore_keys_path = os.path.join(self.cfg.datastore, "keys.npy")
            # datastore_keys_path = "/users10/lhuang/xiaokenaifan/knn-models/examples/knnmt/multi-domain-datastore/law/keys.npy"
        ...
        # self.neg_num_tokens = 0

        # self.pos_example_path = os.path.join(self.cfg.pos_sample, self.cfg.domain, "pos_sample.npy")
        # self.neg_sample_path = os.path.join(self.cfg.neg_sample, self.cfg.domain, "neg_sample.npy" )
        
        # 采集正样例 & 负样例

        # self.pos_example_data = np.memmap(
        #     self.pos_example_path, 
        #     dtype = np.float32 if self.cfg.keys_dtype == "fp32" else np.float16, 
        #     mode = "w+",
        #     shape = (self.cfg.datastore_size * self.cfg.num_neighbors, self.cfg.keys_dimension)
        #     )

        # self.neg_example_data = np.memmap(
        #     self.neg_sample_path,
        #     dtype= np.float32 if self.cfg.keys_dtype == "fp32" else np.float16, 
        #     mode = "w+",
        #     shape = (self.cfg.datastore_size * self.cfg.num_neighbors, self.cfg.keys_dimension)
        #     )
    ...
    def retrieve(self, queries):
        ...
        # collected_keys = queries.index_select(dim=0, index=non_pad_indices)

        queries_device = queries.device

        # B*T x K
        distance, idx = self.index.search(queries.cpu().float().numpy(), self.cfg.num_neighbors)
        # logger.info(len(idx[0]))
        # logger.info(len(distance))
        # logger.info(len(idx))
        # exit()

        # Datastore中检索到k个近邻的隐变量表示 B x k x C
        # hidden_state = torch.from_numpy(self.datastore_keys[idx]).to(queries_device)

        # B*k x C 把检索到的k个近邻的隐变量表示写入
        # pos_example = hidden_state.view(-1, self.cfg.keys_dimension).cpu().numpy()
        # self.pos_example_data[self.num_pos_saved_tokens : self.num_pos_saved_tokens + hidden_state_to_write.shape[0]] = hidden_state_to_write
        # self.num_pos_saved_tokens += hidden_state_to_write.shape[0]
        # self.pos_example_data.flush()

        # logger.info("正样例采集完毕")

        # random_hidden_state = np.random.randint(low = 0, high = self.cfg.datastore_size, size = (1, pos_example.shape[0]), dtype = int)

        # neg_example = self.datastore_keys[random_hidden_state][0]

        # self.neg_example_data[self.num_neg_saved_tokens : self.num_neg_saved_tokens + pos_example.shape[0]] = self.datastore_keys[random_hidden_state][0]
        # self.num_neg_saved_tokens += hidden_state_to_write.shape[0]
        # self.neg_example_data.flush()
        # logger.info("负样例采集完毕")
        del queries
