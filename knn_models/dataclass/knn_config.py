...
@dataclass
class LlmaConfig(FairseqDataclass):
    llma: bool = field(
        default=False,
        metadata={
            "help": "是否选择llma进行生成"
    ...
    llma_criterion: str = field(
        default="prefix",
        metadata={
            "help": "依据前缀匹配 prefix，依据hdz匹配则是hidden"
        }
    )

    llma_prefix_length: int = field(
        default=5,
        metadata={
            "help": "采取前缀匹配的时候，应该检索多长的前缀"
        }
    )
    llma_suffix_length: int = field(
        default=5,
        metadata={
            "help": "采取前缀匹配的时候，应该检索多长的前缀"
        }
    )
    llma_threshold: float = field(
        default=0.8,
        metadata={
            "help": "相似度匹配阈值？这个看着做吧"
        }
